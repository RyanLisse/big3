...
const mockRedisClient = {
  get: vi.fn((_key: string) => Effect.succeed("test value")),
  set: vi.fn((_key: string, _value: string) => Effect.succeed(undefined)),
  del: vi.fn((_key: string) => Effect.succeed(undefined)),
  keys: vi.fn((_pattern: string) => Effect.succeed(["key1", "key2"])),
};

const mockCompositeFilesystem = {
  writeFile: vi.fn(
    (_path: string) => (_content: string) => Effect.succeed(undefined)
  ),
  readFile: vi.fn((_path: string) => Effect.succeed("file content")),
  exists: vi.fn((_path: string) => Effect.succeed(true)),
  delete: vi.fn((_path: string) => Effect.succeed(undefined)),
...
const mockCheckpointer = {
  save: vi.fn((_threadId: string, _checkpoint: any) =>
    Effect.succeed(undefined)
  ),
  load: vi.fn((_threadId: string) => Effect.succeed({ data: "test" })),
...
describe("Persistence Layer", () => {
  ...
  describe("CompositeFilesystem", () => {
    it("should write files to workspace directory", async () => {
      const program = pipe(
        CompositeFilesystem,
        Effect.flatMap((fs) =>
          fs.writeFile("/workspace/test.txt")("test content")
        )
      );
      ...
      expect(mockCompositeFilesystem.writeFile).toHaveBeenCalledWith(
        "/workspace/test.txt"
      );
    });
    ...
    it("should check file existence", async () => {
      ...
      expect(mockCompositeFilesystem.exists).toHaveBeenCalledWith(
        "/workspace/test.txt"
      );
    });
    ...
    it("should delete files", async () => {
      ...
      expect(result).toEqual(undefined);
      expect(mockCompositeFilesystem.delete).toHaveBeenCalledWith(
        "/workspace/test.txt"
      );
    });
    ...
    it("should list files in directory", async () => {
      ...
      expect(mockCompositeFilesystem.listFiles).toHaveBeenCalledWith(
        "/workspace"
      );
    });
  });
  ...
  describe("Checkpointer", () => {
    it("should save checkpoints", async () => {
      ...
      expect(result).toEqual(undefined);
      expect(mockCheckpointer.save).toHaveBeenCalledWith("thread-123", {
        data: "test",
      });
    });
    ...
    it("should list all checkpoints", async () => {
      mockCheckpointer.list.mockReturnValue(
        Effect.succeed(["thread-1", "thread-2"])
      );
  ...
  describe("Integration Tests", () => {
    it("should handle workspace artifact persistence", async () => {
      ...
      const saveProgram = pipe(
        CompositeFilesystem,
        Effect.flatMap((fs) =>
          fs.writeFile(artifact.path)("# Plan\n\nThis is a test plan.")
        )
      );
      ...
      const content = await Effect.runPromise(
        readProgram.pipe(Effect.provide(TestPersistenceLayer))
      );
