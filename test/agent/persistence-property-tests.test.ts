...
import { generateId, validateSessionId, safeJsonParse, safeJsonStringify } from "../../backend/agent/shared/utils.js"
...
describe("Persistence Property-Based Tests", () => {
  ...
  describe("Session Repository Properties", () => {
    ...
    it("should preserve session state through save/load cycle", async () => {
      ...
      const loadedSession = await Effect.runPromise(
        AgentSessionRepo.pipe(
          Effect.flatMap(repo => repo.get(originalSession.id)),
          Effect.provide(TestRedisLayer)
        )
      )
      ...
      const operations = sessions.map(session =>
        Effect.gen(function* (_) {
          yield* AgentSessionRepo.pipe(
            ...
            Effect.flatMap(repo => repo.get(session.id)),
            Effect.flatMap(sessionOption => 
              sessionOption ? Effect.succeed(sessionOption.value) : Effect.fail(new Error("Session not found"))
            )
          )
        })
      )
    ...
    it("should maintain session status transitions", async () => {
      ...
      for (const [fromStatus, toStatuses] of Object.entries(validTransitions)) {
        for (const toStatus of toStatuses) {
          ...
          const updatedSession = { ...session, status: toStatus as AgentSessionStatus, updatedAt: new Date() }
    ...
    it("should handle artifact filtering by session", async () => {
      ...
      mockRedisClient.get.mockImplementation((key) => {
        if (key.includes("keys")) {
          return Promise.resolve(artifacts.map(a => `artifact:${a.id}`).join("\n"))
        }
        const artifact = artifacts.find(a => key.includes(a.id))
        return Promise.resolve(artifact ? JSON.stringify(artifact) : null)
      })
      ...
      const session1Artifacts = await Effect.runPromise(
        WorkspaceArtifactRepo.pipe(
          Effect.flatMap(repo => repo.listBySession(sessionId1)),
          Effect.provide(TestRedisLayer)
        )
      )
      ...
      session1Artifacts.forEach(artifact => {
        expect(artifact.sessionId).toBe(sessionId1)
      })
      
      session2Artifacts.forEach(artifact => {
  ...
  describe("Redis Checkpointer Properties", () => {
    it("should handle checkpoint serialization/deserialization", async () => {
      ...
      // Mock Redis operations
      mockRedisSaver.save.mockResolvedValue(undefined)
      mockRedisSaver.load.mockImplementation((threadId) => {
        const checkpoint = checkpoints.find(cp => cp.thread_id === threadId)
        return Promise.resolve(checkpoint?.checkpoint)
      })

      // Test save/load cycles
      for (const checkpointData of checkpoints) {
        await Effect.runPromise(
          Effect.succeed(mockRedisSaver).pipe(
            Effect.flatMap(saver => saver.save(checkpointData.thread_id, checkpointData.checkpoint))
          )
        )

        const loadedCheckpoint = await Effect.runPromise(
          Effect.succeed(mockRedisSaver).pipe(
            Effect.flatMap(saver => saver.load(checkpointData.thread_id))
          )
        )
    ...
    it("should maintain checkpoint ordering", async () => {
      ...
      // Mock Redis to return checkpoints in order
      mockRedisSaver.save.mockResolvedValue(undefined)
      mockRedisSaver.load.mockResolvedValue(checkpoints[numCheckpoints - 1]) // Return latest

      // Save checkpoints in order
      for (let i = 0; i < numCheckpoints; i++) {
        await Effect.runPromise(
          Effect.succeed(mockRedisSaver).pipe(
            Effect.flatMap(saver => saver.save(threadId, checkpoints[i]))
          )
        )
      }
      ...
      const latestCheckpoint = await Effect.runPromise(
        Effect.succeed(mockRedisSaver).pipe(
          Effect.flatMap(saver => saver.load(threadId))
        )
      )
  ...
  describe("JSON Serialization Properties", () => {
    it("should handle various data types safely", async () => {
      const testData = [
        ...
        { date: new Date().toISOString(), null: null, undefined: undefined },
  ...
  describe("Performance Properties", () => {
    it("should handle large payloads efficiently", async () => {
      ...
      const jsonString = await Effect.runPromise(safeJsonStringify(largePayload, "large_payload_test"))
      const serializationTime = Date.now() - startTime

      expect(jsonString.length).toBeGreaterThan(100000) // Should be > 100KB
      expect(serializationTime).toBeLessThan(1000) // Should complete within 1 second

      // Test deserialization
      const deserializationStart = Date.now()
      const parsedData = await Effect.runPromise(safeJsonParse(jsonString, "large_payload_test"))
    ...
    it("should maintain performance under concurrent load", async () => {
      ...
      const operations = payloads.map((payload, index) =>
        Effect.gen(function* (_) {
          const jsonString = yield* safeJsonStringify(payload, `concurrent_test_${index}`)
          yield* Effect.succeed(mockRedisClient).pipe(
            Effect.flatMap(client => Effect.tryPromise(() => client.set(`test_${index}`, jsonString)))
          )
          const loadedJson = yield* Effect.succeed(mockRedisClient).pipe(
            Effect.flatMap(client => Effect.tryPromise(() => client.get(`test_${index}`)))
          )
          const parsed = yield* safeJsonParse(loadedJson || "", `concurrent_test_${index}`)
          return parsed
        })
      )
