...
describe("Failure Path Tests - User Story 1", () => {
  ...
  describe("Tool Failure Scenarios", () => {
    it("should handle Claude service failure with clear error message", async () => {
      ...
      const program = Effect.gen(function* (_) {
        const result = yield* Effect.tryPromise(() =>
          ...
          Effect.map((analysis) => ({ success: true, analysis })),
          Effect.catchAll(() =>
            Effect.succeed({
              success: false,
              error: "Claude service unavailable",
              message:
                "Unable to analyze code at this time. Please try again later.",
              details: "Anthropic API rate limit exceeded",
              alternative:
                "You can manually review the code or try a different approach.",
            })
          )
        );
        return result;
    ...
    it("should handle Gemini/Playwright failure with fallback", async () => {
      ...
      const browserError = new Error(
        "Playwright timeout: Navigation timeout exceeded"
      );
      mockBrowserService.navigate.mockRejectedValue(browserError);

      const program = Effect.gen(function* (_) {
        const result = yield* Effect.tryPromise(() =>
          mockBrowserService.navigate("https://example.com")
        ).pipe(
          ...
          Effect.catchAll(() =>
            Effect.succeed({
              ...
              message:
                "Unable to access the requested webpage. The page may be unavailable or taking too long to load.",
              details: "Playwright timeout: Navigation timeout exceeded",
              fallback:
                "Here's what we know about the topic based on existing information...",
            })
          )
        );
        return result;
      ...
      expect(result.message).toContain(
        "Unable to access the requested webpage"
      );
    ...
    it("should handle OpenAI Realtime API failure", async () => {
      ...
      const program = Effect.gen(function* (_) {
        const result = yield* Effect.tryPromise(() =>
          ...
          Effect.map((transcription) => ({ success: true, transcription })),
          Effect.catchAll(() =>
            Effect.succeed({
              success: false,
              error: "Voice service unavailable",
              message:
                "Unable to process audio input. Please use text input instead.",
              details: "OpenAI API connection refused",
              suggestion:
                "You can type your request directly, and I'll process it without voice input.",
            })
          )
        );
        return result;
    ...
    it("should handle multiple concurrent tool failures", async () => {
      ...
      mockBrowserService.extractContent.mockRejectedValue(
        new Error("Content extraction failed")
      );
      mockVoiceService.speak.mockRejectedValue(
        ...
        const tools = [
          {
            ...
            fn: () => mockCoderService.generateCode("component.tsx"),
          },
          {
            name: "Content Extraction",
            fn: () => mockBrowserService.extractContent("url"),
          },
          {
            name: "Text-to-Speech",
            fn: () => mockVoiceService.speak("Hello world"),
          },
        ];
  ...
  describe("System Stability Under Failure", () => {
    it("should maintain session stability when tools fail", async () => {
      ...
      mockCoderService.executeCode.mockRejectedValueOnce(
        ...
        const execution = yield* Effect.tryPromise(() =>
          ...
          Effect.catchAll(() =>
            Effect.succeed({
              ...
              sessionStable: true,
              fallback:
                "Code execution failed, but here's what would happen: 'test' would be logged",
            })
          )
        );
        return execution;
    ...
    it("should recover from transient failures", async () => {
      ...
      const program = Effect.gen(function* (_) {
        ...
        while (attempts < maxAttempts) {
          attempts++;
          const navResult = yield* Effect.tryPromise(() =>
            mockBrowserService.navigate("https://example.com")
          ).pipe(
          ...
          if (navResult.success) {
            return { success: true, result: navResult.result, attempts };
          }
  ...
  describe("User Communication During Failures", () => {
    it("should provide clear, actionable error messages", async () => {
      mockCoderService.analyzeCode.mockRejectedValue(
        new Error("Token limit exceeded")
      );

      const program = Effect.gen(function* (_) {
        const result = yield* Effect.tryPromise(() =>
          mockCoderService.analyzeCode("large-code-file")
        ).pipe(
          ...
          Effect.catchAll(() =>
            Effect.succeed({
              success: false,
              userMessage:
                "I'm unable to analyze this large code file due to token limits.",
              technicalDetails: "Token limit exceeded",
              suggestions: [
