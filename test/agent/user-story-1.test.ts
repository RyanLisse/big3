...
import { AgentSession, AgentSessionStatus } from "../../backend/agent/domain.js"
...
describe("User Story 1 - Voice-driven coding assistant", () => {
  ...
  describe("Independent Test: End-to-end single session", () => {
    it("should process a well-scoped request end-to-end", async () => {
      ...
      const mockProcessingFlow = Effect.gen(function* (_) {
        ...
        const spawnResponse = yield* Effect.sync(() => mockAgentService.spawnAgent({
        ...
        return {
          ...
          artifacts: [
            { type: "analysis", content: "Modular structure could be improved" },
            { type: "suggestion", content: "Add error handling for edge cases" }
          ]
        }
      })
    ...
    it("should handle text-based input without requiring audio", async () => {
      ...
      const program = Effect.gen(function* (_) {
        const response = yield* Effect.sync(() => mockAgentService.spawnAgent(textRequest))
  ...
  describe("Tool Integration", () => {
    it("should integrate Voice, Coder, and Browser services", async () => {
      // Mock tool responses
      mockVoiceService.transcribe.mockResolvedValue("Analyze the performance issues in this code")
      mockCoderService.analyzeCode.mockResolvedValue({
        issues: ["Memory leak in loop", "Inefficient algorithm"],
        suggestions: ["Use proper cleanup", "Optimize with better data structure"]
      })
      ...
      const program = Effect.gen(function* (_) {
        ...
        const research = yield* Effect.sync(() => mockBrowserService.extractContent("https://docs.example.com"))
        
        return {
          transcription,
          analysis,
          research,
          combinedResult: "Analysis complete with voice input, code review, and research"
        }
      })
      ...
      expect(result.transcription).toBe("Analyze the performance issues in this code")
    ...
    it("should return at least one concrete result", async () => {
      const program = Effect.gen(function* (_) {
        ...
        const results = [
          { type: "code", content: "function optimized() { /* improved logic */ }" },
          { type: "explanation", content: "The function was optimized for better performance" },
  ...
  describe("Error Handling and Graceful Degradation", () => {
    it("should handle tool failures gracefully", async () => {
      // Mock a tool failure
      mockCoderService.executeCode.mockRejectedValue(new Error("Code execution failed"))
      ...
      const program = Effect.gen(function* (_) {
        try {
          ...
          return { status: "success", result: "Code executed" }
        } catch (error) {
          // Graceful degradation - provide alternative result
          return {
            status: "degraded",
    ...
    it("should inform user when tool calls fail", async () => {
      mockVoiceService.processAudio.mockRejectedValue(new Error("Audio processing failed"))

      const program = Effect.gen(function* (_) {
        try {
          yield* Effect.sync(() => mockVoiceService.processAudio("audio-data"))
          return { success: true, message: "Audio processed successfully" }
        } catch (error) {
          return {
            success: false,
            message: "Audio processing failed. Please try again or use text input.",
            error: error instanceof Error ? error.message : "Unknown error"
          }
        }
      })
    ...
    it("should maintain session state throughout interaction", async () => {
      ...
      const program = Effect.gen(function* (_) {
        ...
        for (const interaction of interactions) {
          ...
          results.push(`Processed: ${interaction} for session ${spawnResponse.sessionId}`)
        }
  ...
  describe("Acceptance Criteria Validation", () => {
    it("should meet acceptance scenario 1: Clear request with coherent response", async () => {
      const request = "Summarize this architecture document and propose implementation phases"
      ...
      const program = Effect.gen(function* (_) {
        const response = yield* Effect.sync(() => mockAgentService.spawnAgent({
      ...
      return true
        ...
        const plan = {
          summary: "Architecture consists of 3 main layers: presentation, business logic, and data",
          phases: [
        ...
        return {
          sessionId: response.sessionId,
          plan,
          meetsCriteria: !!plan.summary && !!plan.phases && !!plan.concreteResult
        }
      })
    ...
    it("should meet acceptance scenario 2: Handle tool failure gracefully", async () => {
      // Mock tool failure
      mockCoderService.generateCode.mockRejectedValue(new Error("Code generation timeout"))

      const program = Effect.gen(function* (_) {
        try {
          yield* Effect.sync(() => mockCoderService.generateCode("component.tsx"))
