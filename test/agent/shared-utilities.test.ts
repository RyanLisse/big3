...
import {
  ...
  STEP_STATUSES,
  TOOLS,
} from "../../backend/agent/shared/constants.js";
import {
  isCompleted,
  isFailed,
  isPending,
...
describe("Shared Utilities Tests", () => {
  ...
  describe("Entity Creation", () => {
    ...
    it("should create valid plans", () => {
      ...
      const plan = createPlan("plan-123", "Test Plan", steps, {
        priority: "high",
      });
  ...
  describe("Performance Properties", () => {
    it("should handle large objects efficiently", async () => {
      ...
      const jsonString = await Effect.runPromise(
        safeJsonStringify(largeObject, "large_test")
      );
      ...
      const parsedData = await Effect.runPromise(
        safeJsonParse(jsonString, "large_test")
      );
