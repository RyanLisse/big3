...
describe("Model Registry Enhancement", () => {
  ...
  describe("Model Compatibility Validation", () => {
    ...
    it("should reject incompatible models", () => {
      const compatibility = registry.validateModelCompatibility(
        "non-existent-model",
        {}
      );
  ...
  describe("Provider Registry Management", () => {
    ...
    it("should get all available models by provider", () => {
      const openaiModels = registry
        .listModels()
        .filter((m) => m.provider === "openai");
      const googleModels = registry
        .listModels()
        .filter((m) => m.provider === "google");
      const anthropicModels = registry
        .listModels()
  ...
  describe("Registry Performance", () => {
    ...
    it("should handle multiple compatibility checks", () => {
      ...
      registry.validateModelCompatibility("claude-3.5", {
        minContextWindow: 100_000,
      });
      registry.validateModelCompatibility("gemini-pro", {
        minContextWindow: 30_000,
      });
