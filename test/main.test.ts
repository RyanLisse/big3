...
import type { FunctionCallEvent, RealtimeEvent } from "../src/domain.js";
...
// Mock Layer Factory
const createMockVoiceLayer = (
  events: RealtimeEvent[],
  capturedMessages: RealtimeEvent[]
) => {
  return Layer.effect(
    VoiceService,
    Effect.gen(function* () {
...
const createMockBrowserLayer = (actionResponses: Map<string, string>) => {
  ...
  return Layer.succeed(BrowserServiceTag, {
    navigate: (url: string) =>
      Effect.sync(() => {
        currentUrl = url;
      }),
    act: (instruction: string) =>
      Effect.succeed(
        actionResponses.get(instruction) ||
          `Mock action: ${instruction} on ${currentUrl}`
      ),
  });
};
...
const createTestableMainProgram = (maxIterations: number) =>
  Effect.gen(function* () {
    ...
    const coder = yield* CoderServiceTag;
    const browser = yield* BrowserServiceTag;
    ...
    while (iterations < maxIterations) {
      ...
      if (event.type === "response.function_call_arguments.done") {
        ...
        if (functionName === "command_agent") {
          const result = yield* coder.execute(
            "default-session",
            args.instruction
          );
          yield* voice.send({
          ...
          if (args.url) {
            yield* browser.navigate(args.url);
          }
...
describe("MainProgram Integration Tests", () => {
  it.effect("should initialize and send session.update with tools", () =>
    Effect.gen(function* () {
      ...
      expect((capturedMessages[0] as any).session.tools[0].name).toBe(
        "command_agent"
      );
      expect((capturedMessages[0] as any).session.tools[1].name).toBe(
        "browser_use"
      );
    })
  );
  ...
  it.effect("should handle browser_use function call with navigation", () =>
    Effect.gen(function* () {
      ...
      expect((capturedMessages[1] as any).item.output).toContain(
  ...
  it.effect("should process multiple function calls in sequence", () =>
    Effect.gen(function* () {
      ...
      expect(
        capturedMessages.filter((m) => m.type === "conversation.item.create")
      ).toHaveLength(3);
      expect(
        capturedMessages.filter((m) => m.type === "response.create")
      ).toHaveLength(3);
    })
  );
  ...
  it.effect("should handle coder service errors", () =>
    Effect.gen(function* () {
      ...
      const failingCoderLayer = Layer.succeed(CoderServiceTag, {
        createSession: (name: string) => Effect.succeed(`Session: ${name}`),
        execute: (_sessionId: string, _instruction: string) =>
  ...
  it.effect("should handle browser service navigation errors", () =>
    Effect.gen(function* () {
      ...
      const failingBrowserLayer = Layer.succeed(BrowserServiceTag, {
        navigate: (_url: string) =>
          Effect.fail(new Error("Navigation failed: invalid URL")),
        act: (_instruction: string) => Effect.succeed("ok"),
      });
  ...
  it.effect("should handle browser service action errors", () =>
    Effect.gen(function* () {
      ...
      const failingBrowserLayer = Layer.succeed(BrowserServiceTag, {
        navigate: (_url: string) => Effect.void,
        act: (_instruction: string) =>
          Effect.fail(new Error("Action failed: element not found")),
      });
  ...
  it.effect(
    "should route function calls correctly based on function name",
    () =>
      Effect.gen(function* () {
        // Arrange
        const capturedMessages: RealtimeEvent[] = [];
        ...
        const trackingCoderLayer = Layer.succeed(CoderServiceTag, {
        ...
        const trackingBrowserLayer = Layer.succeed(BrowserServiceTag, {
          navigate: (_url: string) => Effect.void,
          act: (_instruction: string) =>
            Effect.sync(() => {
              browserCalled = true;
