...
import { MultiAgentTeam, TeamNotFoundError } from "../src/domain.js"
import {
  MultiAgentService,
  TeamRepository,
...
const mockTeamRepository = {
  ...
  update: (id: string, request) => Effect.succeed(undefined),
  delete: (id: string) => Effect.succeed(undefined),
  list: (filters) => Effect.succeed([])
}

const mockMembershipRepository = {
  addAgent: (teamId: string, agentId: string, role?: string) => Effect.succeed(undefined),
  removeAgent: (teamId: string, agentId: string) => Effect.succeed(undefined),
  getTeamAgents: (teamId: string) => Effect.succeed([]),
  getAgentTeams: (agentId: string) => Effect.succeed([])
}
...
const mockMemoryRepository = {
  ...
  delete: (id: string) => Effect.succeed(undefined),
  listByTeam: (teamId: string) => Effect.succeed([]),
  checkAccess: (agentId: string, blockId: string, action) => Effect.succeed(true),
  logAccess: (log) => Effect.succeed(undefined)
}
...
const mockMessageRepository = {
  ...
  updateStatus: (id: string, status) => Effect.succeed(undefined),
  getAgentMessages: (agentId: string, filters) => Effect.succeed([]),
  getTeamMessages: (teamId: string, filters) => Effect.succeed([]),
  markAsRead: (id: string) => Effect.succeed(undefined)
}
...
const mockToolRuleRepository = {
  ...
  delete: (id: string) => Effect.succeed(undefined),
  listByTeam: (teamId: string) => Effect.succeed([]),
  getActiveRulesForTool: (teamId: string, toolName: string) => Effect.succeed([])
}

const mockMessageQueue = {
  enqueue: (message) => Effect.succeed(undefined),
  dequeue: (agentId: string) => Effect.succeed(Option.none()),
  broadcast: (teamId: string, message) => Effect.succeed([])
}
...
const MockMessageQueueLayer = Layer.succeed(
  MessageQueue,
  mockMessageQueue
)
...
const MockMultiAgentServiceLayer = Layer.effect(
  ...
  Effect.gen(function*(_) {
    ...
    const messageQueue = yield* _(MessageQueue)

    return {
      createTeam: teamRepo.create,
      getTeam: (id: string) => Effect.gen(function*(_) {
      ...
      getSharedMemory: (id: string, agentId: string) => Effect.gen(function*(_) {
      ...
      getToolRules: (teamId: string, agentId: string) => toolRuleRepo.listByTeam(teamId)
    }
  })
)
...
describe("MultiAgentService", () => {
  it("should create a team successfully", async () => {
    ...
    const result = await Effect.runPromise(program.pipe(Effect.provide(MockMultiAgentLayers)))
  ...
  it("should retrieve shared memory by ID", async () => {
    const program = Effect.gen(function*(_) {
      const service = yield* _(MultiAgentService)
      const result = yield* _(service.getSharedMemory("memory-123", "agent-123"))
  ...
  it("should update shared memory successfully", async () => {
    const program = Effect.gen(function*(_) {
      const service = yield* _(MultiAgentService)
      const result = yield* _(service.updateSharedMemory("memory-123", {
  ...
  it("should validate tool execution", async () => {
    const program = Effect.gen(function*(_) {
      const service = yield* _(MultiAgentService)
      const result = yield* _(service.validateToolExecution("team-123", "test_tool", "agent-123"))
  ...
  it("should list shared memory by team successfully", async () => {
    const program = Effect.gen(function*(_) {
      const service = yield* _(MultiAgentService)
      const result = yield* _(service.listSharedMemoryByTeam("team-123", "agent-123"))
