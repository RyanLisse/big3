...
import type { AgentConfig, ModelRegistry, SDKConfig } from "../config/types";
...
export class AIAgentSDK {
  ...
  constructor(config: SDKConfig) {
    ...
    if (config.registry && typeof config.registry !== "string") {
      ...
      if (typeof config.registry === "string") {
        // Registry path loading not implemented, using default
      }
    }
  }
  ...
  createAgent(request: CreateAgentRequest): Agent {
    const model =
      request.model || request.config.model || this.registry.getDefaultModel();
  ...
  switchModel(request: ModelSwitchRequest): Agent {
    ...
    const compatibility = this.registry.validateModelCompatibility(
      request.model,
      {}
    );
    if (!compatibility.compatible) {
      // Model switch may have compatibility issues
    }
