...
export interface CliCommand {
  ...
  readonly execute: (args: ReadonlyArray<string>) => Effect.Effect<void, never>
}
...
export interface MultiAgentCliService {
  readonly executeCommand: (commandName: string, args: ReadonlyArray<string>) => Effect.Effect<void, never>
}

// CLI Service Tag
export const MultiAgentCliService = Context.GenericTag<MultiAgentCliService>("MultiAgentCliService")
...
class MultiAgentCliServiceImpl implements MultiAgentCliService {
  ...
  ): { options: CliOptions; remainingArgs: ReadonlyArray<string> } => {
  ...
  // Format output
  private formatOutput = <T>(data: T, options: CliOptions): string => {
    if (options.json) {
  ...
  private formatObject = (obj: unknown, indent = 0): string => {
    const spaces = "  ".repeat(indent)
    if (Array.isArray(obj)) {
      return obj.map((item) => `${spaces}- ${this.formatValue(item, indent + 1)}`).join("\n")
    } else if (typeof obj === "object" && obj !== null) {
      return Object.entries(obj)
        .filter(([_, value]) => value !== undefined && value !== null)
        .map(([key, value]) => `${spaces}${key}: ${this.formatValue(value, indent + 1)}`)
        .join("\n")
    }
    return String(obj)
  }
  ...
  // Print error
  private printError = (error: string, options: CliOptions): void => {
    if (options.json) {
      console.error(JSON.stringify({ error, timestamp: new Date().toISOString() }, null, 2))
    } else {
  ...
  // Print success
  private printSuccess = (message: string, options: CliOptions): void => {
    if (options.json) {
      console.log(JSON.stringify({ success: true, message, timestamp: new Date().toISOString() }, null, 2))
    } else {
      console.log(`✅ ${message}`)
    }
  }
  ...
  // Team Commands
  private createTeamCommand = (): CliCommand => ({
    name: "create-team",
      ...
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length < 1) {
            return Effect.succeed(this.showHelp("create-team", [
          ...
          return pipe(
            ...
            Effect.flatMap((response) =>
              response.success && response.data
                ? Effect.succeed(this.printSuccess(`Team created: ${(response.data as any).id}`, options))
                : Effect.succeed(this.printError(response.error?.message || "Unknown error", options))
            ),
            Effect.catchAll((error) => Effect.succeed(this.printError(String(error), options)))
          )
        })
      )
  })

  private listTeamsCommand = (): CliCommand => ({
    name: "list-teams",
    ...
    execute: (args) =>
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help) {
            return Effect.succeed(this.showHelp("list-teams", [
          ...
          return pipe(
            ...
            Effect.flatMap((response) =>
              response.success && response.data
                ? Effect.succeed(console.log(this.formatOutput(response.data, options)))
                : Effect.succeed(this.printError(response.error?.message || "Unknown error", options))
            ),
            Effect.catchAll((error) => Effect.succeed(this.printError(String(error), options)))
          )
  ...
  private getTeamCommand = (): CliCommand => ({
    name: "get-team",
    ...
    execute: (args) =>
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length !== 1) {
            return Effect.succeed(this.showHelp("get-team", [
      ...
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length < 4) {
            return Effect.succeed(this.showHelp("create-memory", [
          ...
          const [teamId, label, type, value, ...descriptionParts] = remainingArgs
          const description = descriptionParts.join(" ")
          ...
          return pipe(
            ...
            Effect.flatMap((response) =>
              response.success && response.data
                ? Effect.succeed(this.printSuccess(`Memory created: ${(response.data as any).id}`, options))
                : Effect.succeed(this.printError(response.error?.message || "Unknown error", options))
            ),
            Effect.catchAll((error) => Effect.succeed(this.printError(String(error), options)))
          )
  ...
  private listMemoryCommand = (): CliCommand => ({
    name: "list-memory",
    ...
    execute: (args) =>
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length < 1) {
            return Effect.succeed(this.showHelp("list-memory", [
  ...
  // Message Commands
  private sendMessageCommand = (): CliCommand => ({
    name: "send-message",
      ...
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length < 3) {
            return Effect.succeed(this.showHelp("send-message", [
          ...
          const priorityIndex = messageParts.findIndex((part) => ["low", "normal", "high", "urgent"].includes(part))
          ...
          return pipe(
            ...
            Effect.flatMap((response) =>
              response.success && response.data
                ? Effect.succeed(this.printSuccess(`Message sent: ${(response.data as any).id}`, options))
                : Effect.succeed(this.printError(response.error?.message || "Unknown error", options))
            ),
            Effect.catchAll((error) => Effect.succeed(this.printError(String(error), options)))
          )
  ...
  private listMessagesCommand = (): CliCommand => ({
    name: "list-messages",
    ...
    execute: (args) =>
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length < 1) {
            return Effect.succeed(this.showHelp("list-messages", [
  ...
  private addAgentCommand = (): CliCommand => ({
    name: "add-agent",
      ...
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length < 2) {
            return Effect.succeed(this.showHelp("add-agent", [
          ...
          return pipe(
            ...
            Effect.flatMap((response) =>
              response.success
                ? Effect.succeed(this.printSuccess(`Agent ${agentId} added to team ${teamId}`, options))
                : Effect.succeed(this.printError(response.error?.message || "Unknown error", options))
            ),
            Effect.catchAll((error) => Effect.succeed(this.printError(String(error), options)))
          )
  ...
  private removeAgentCommand = (): CliCommand => ({
    name: "remove-agent",
    ...
    execute: (args) =>
      pipe(
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          if (options.help || remainingArgs.length !== 2) {
            return Effect.succeed(this.showHelp("remove-agent", [
          ...
          return pipe(
            ...
            Effect.flatMap((response) =>
              response.success
                ? Effect.succeed(this.printSuccess(`Agent ${agentId} removed from team ${teamId}`, options))
                : Effect.succeed(this.printError(response.error?.message || "Unknown error", options))
            ),
            Effect.catchAll((error) => Effect.succeed(this.printError(String(error), options)))
          )
  ...
  // Help command
  private helpCommand = (): CliCommand => ({
    name: "help",
        ...
        Effect.flatMap(({ options, remainingArgs }) => {
          ...
          if (commandName) {
            ...
            return Option.match(commandOpt, {
              onSome: (command) => Effect.succeed(this.showHelp(commandName, [
  ...
  // Get all commands
  private getCommands = (): ReadonlyArray<CliCommand> => [
  ...
  private getCommand = (name: string): Option.Option<CliCommand> => {
    const commands = this.getCommands()
    return A.findFirst(commands, (cmd) => cmd.name === name)
  }

  private getUsageForCommand = (command: CliCommand): string => {
    switch (command.name) {
      case "create-team":
  ...
  private showHelp = (command: string, lines: ReadonlyArray<string>): void => {
  ...
  private showMainHelp = (): void => {
    ...
    console.log("  --help, -h     Show this help for commands")
    console.log("\nExamples:")
    console.log("  multi-agent create-team \"Development Team\" \"Team for web development\"")
    console.log("  multi-agent add-agent team-123 agent-456 developer")
    console.log("  multi-agent create-memory team-123 requirements project_context \"User authentication required\"")
    console.log("  multi-agent send-message agent-1 agent-2 \"Please review the API design\" high")
    console.log("  multi-agent list-messages agent-1 --team delivered")
  }

  // Execute command
  executeCommand = (commandName: string, args: ReadonlyArray<string>): Effect.Effect<void, never> =>
    pipe(
...
export const MultiAgentCliServiceLive = Layer.effect(
  ...
  Effect.map(MultiAgentApiService, (apiService) => new MultiAgentCliServiceImpl(apiService))
)
