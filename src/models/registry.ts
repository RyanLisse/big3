...
export function validateModelCompatibility(
  ...
  return fromInfo.compatibility.some((tag) =>
    toInfo.compatibility.includes(tag)
  );
}
...
function isValidModelRequirements(req: unknown): req is ModelRequirements {
  ...
  return (
    (r.minContextWindow === undefined ||
      typeof r.minContextWindow === "number") &&
    (r.minMaxOutputTokens === undefined ||
      typeof r.minMaxOutputTokens === "number") &&
    (r.requiredCapabilities === undefined ||
      Array.isArray(r.requiredCapabilities))
  );
}

function isCapabilitySupported(
  cap: unknown,
  capList: readonly string[]
): cap is string {
...
export class ModelRegistryImpl implements ModelRegistry {
  private readonly registry: ModelRegistryData;
  ...
  getModelsByProvider(provider: string): ModelInfo[] {
    return Object.values(this.registry.models).filter(
