...
export interface ModelRegistry {
  ...
  validateModelCompatibility(modelId: string, requirements: Record<string, any>): ModelCompatibility
  getDefaultModel(): string
  getAvailableModels(): string[]
}

export interface ModelRegistryData {
  models: Record<string, ModelInfo & { capabilities: string[], compatibility: string[], version: string }>
  default: string
}
...
export const SDKConfigSchema = z.object({
  ...
  model: z.string().optional(),
  registry: z.custom<ModelRegistry>((val) => val !== null && typeof val === 'object').optional(),
...
export const validateAgentConfig = (config: unknown) => AgentConfigSchema.safeParse(config)
...
export const isValidRegistry = (registry: unknown): registry is string => {
  return typeof registry === 'string' && registry.trim().length > 0
}
...
export const isValidLoggingConfig = (logging: unknown): logging is object => {
  return logging !== null && typeof logging === 'object' && 
    'level' in logging && 
